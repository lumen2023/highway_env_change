...
class MergeEnv(AbstractEnv):
    ...
    def _make_road(self) -> None:
        ...
        y = [4, 0, -4, -8]
        "c:连续 s：虚线 n：无线"
        line_type = [[c, s], [n, c], [c, c]] # 实虚线 单实线 双实线
        line_type_merge = [[c, s], [n, s]]   # 实虚线 单实线
        a = line_type[0]
        # main_lanes
        ...
        for i in range(self.config["main_lanes_count"]):
            net.add_lane("a", "b", StraightLane([0, y[i]], [sum(ends[:2]), y[i]], 
                                                line_types=line_type[2], speed_limit=30))
            net.add_lane("b", "c", StraightLane([sum(ends[:2]), y[i]], [sum(ends[:3]), y[i]], 
                                                line_types=line_type_merge[0], speed_limit=30))
            net.add_lane("c", "d",
    ...
    def _make_vehicles(self) -> None:
        ...
        ego_vehicle = self.action_type.vehicle_class(self.road,
                                                     self.road.network.get_lane(("k", "b", 0)).position(0, 0),
                                                     # self.config["main_lanes_count"]
