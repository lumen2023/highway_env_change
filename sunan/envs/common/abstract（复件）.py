...
class AbstractEnv(gym.Env):
    ...
    def _info(self, obs: Observation, action: Optional[Action] = None) -> dict:
        ...
        :return: info dict
        """
        # print(self.vehicle.speed, 0.05 * np.clip(utils.lmap(self.vehicle.speed * np.cos(self.vehicle.heading), self.config["cost_speed_range"], [0, 1]), 0, 1))
        info = {
    ...
    def _compute_headway_distance(self, vehicle):
        ...
        if len(f_d) < 2 and len(r_d) < 2:  # 自车前小于2辆车，自车后小于2辆车
            ...
            if len(f_d) == 0 and len(f_d) == 0:
                Norm_Headway_cost = 1
                Target_v = 25
        # print(Norm_Headway_cost, Target_v)
