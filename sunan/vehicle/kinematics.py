...
class Vehicle(RoadObject):
    ...
    def create_random(cls, road: Road,
        ...
        if speed is None:
            if lane.speed_limit is not None:
                ...
                # speed = road.np_random.uniform(0.5*lane.speed_limit, 1.5*lane.speed_limit)
            else:
    ...
    def create_random_merge(cls, road: Road,
        ...
        if speed is None:
            if lane.speed_limit is not None:
                speed = road.np_random.uniform(0.7*lane.speed_limit, 0.8*lane.speed_limit)
                # speed = road.np_random.uniform(0.8 * lane.speed_limit, 1 * lane.speed_limit)
            else:
                speed = road.np_random.uniform(Vehicle.DEFAULT_INITIAL_SPEEDS[0], Vehicle.DEFAULT_INITIAL_SPEEDS[1])
        default_spacing = 12+1.0*speed
        offset = spacing * default_spacing * np.exp(-5 / 40 * len(road.network.graph[_from][_to]))
    ...
    def to_dict(self, origin_vehicle: "Vehicle" = None, observe_intentions: bool = True) -> dict:
        ...
        sigma_vx = 2
        sigma_vy = 0.2
        nosie_x = np.random.normal(0, sigma_x * noise_p)
        nosie_y = np.random.normal(0, sigma_y * noise_p)
        nosie_vx = np.random.normal(0, sigma_vx * noise_p)
        nosie_vy = np.random.normal(0, sigma_vy * noise_p)
        # self.position0 = self.position[0] + nosie_x
        # self.position1 = self.position[1] + nosie_y
        # self.velocity0 = self.velocity[0] + nosie_vx
        # self.velocity1 = self.velocity[1] + nosie_vy
        # self.heading1 =  self.heading + nosie_h
        # self.direction0 = np.cos(self.heading1)
        # self.direction1 = np.cos(self.heading1)
        d = {
            'presence': 1,
            'x': self.position[0] + nosie_x,
            'y': self.position[1] + nosie_y,
            'vx': self.velocity[0] + nosie_vx,
            'vy': self.velocity[1] + nosie_vy,
